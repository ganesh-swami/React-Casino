...
const GameState = ({ history, children }) => {
  ...
  // const {getCurrentTable} = useContext(tableContext)
  ...
  useEffect(() => {
    if (turn && !turnTimeOutHandle) {
      const handle = setTimeout(fold, 25000);
      setHandle(handle);
    } else {
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.on(TABLE_CREATED, ({ tables, tableId }) => {
        console.log(TABLE_CREATED, tables, tableId);
  ...
  const createTable = (limit=1000, seat=5) => {
    console.log(CREATE_TABLE, limit, seat);
    socket.emit(CREATE_TABLE, limit, seat);
  };
